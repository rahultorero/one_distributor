...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  Future<void> _sendPostRequest() async {
    ...
    print(formData.selectedCode);

    final String apiUrl = ApiConfig.postRegister();
    var request = http.MultipartRequest('POST', Uri.parse(apiUrl));

    // Add fields with integer values, converting them to strings for the request
    request.fields['b_typeid'] = '1';
    request.fields['b_name'] = formData.businessName;
    request.fields['add1'] = formData.address1 ?? '';
    request.fields['add2'] = formData.address2 ?? '';
    request.fields['area'] = formData.selectedArea ?? '';
    request.fields['city'] = formData.selectedCity ?? '';

    // Convert integer fields to strings
    request.fields['pincode'] = formData.pincode?.toString() ?? '';
    request.fields['bstate'] = '60' ?? '';
    request.fields['contactno'] = formData.contactNo?.toString() ?? '';

    request.fields['pname'] = formData.personName ?? '';
    request.fields['email'] = formData.email ?? '';
    request.fields['gstin'] = formData.gstNo ?? '';
    request.fields['pan'] = formData.panNo ?? '';
    request.fields['dlno'] = formData.drugLicenceNo ?? '';
    request.fields['b_type'] = formData.selectedIndustry ?? '';

    // Add document files from uploadedDocuments
    final documentTypes = {
      'druglic': 'Drug License',
      'fssai': 'Food License',
      'gstcer': 'GST Certificate',
      'udyam': 'Udyam Certificate'
    };

    for (var entry in documentTypes.entries) {
      String documentKey = entry.key;
      String documentType = entry.value;

      // Get the file path from formData.uploadedDocuments
      String? filePath = formData.uploadedDocuments[documentType];
      if (filePath != null) {
        // Add the file to the request
        request.files.add(await http.MultipartFile.fromPath(
          documentKey,
          filePath,
          contentType: MediaType('image', 'png'), // Adjust content type based on file type
        ));
      }
    }
    ...
    try {
      ...
      if (response.statusCode == 201) {
        AwesomeDialog(
          ...
          showCloseIcon: false,
          title: 'Success',
          desc: 'You have successfully completed the sign-up process.',
          btnOkOnPress: () {
...
class _BusinessDetailsScreenState extends State<BusinessDetailsScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _businessNameTextController = TextEditingController(text: _businessName ?? '');
    _address1TextController = TextEditingController(text: _address1 ?? '');
    _address2TextController = TextEditingController(text: _address2 ?? '');
    _pincodeTextController = TextEditingController(text: _pincode ?? '');
    _contactNoTextController = TextEditingController(text: _contactNo ?? '');
    _emailTextController = TextEditingController(text: _email ?? '');
    _personNameTextController = TextEditingController(text: _personName ?? '');
    _gstNoTextController = TextEditingController(text: _gstNo ?? '');
    _panNoTextController = TextEditingController(text: _panNo ?? '');
    _drugLicenceNoTextController = TextEditingController(text: _drugLicenceNo ?? '');
    _fssaiNoTextController = TextEditingController(text: _fssaiNo ?? '');
    _udyamNoTextController = TextEditingController(text: _udyamNo ?? '');
...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  void initState() {
    super.initState();
    final formData = Provider.of<FormDataProvider>(context, listen: false).formData;
    _selectedCheckboxIndustry = formData.selectedIndustry;
    _updateDropdownItems();
    if (formData.selectedCategory != null) {
      _selectedIndustry = _currentDropdownItems.firstWhere(
            (category) => category.fxdsubname == formData.selectedCategory,
